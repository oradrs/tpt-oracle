...
--       ||' 0' -- argument5/report_name. 0 = use awrrpti/awrgrpi defaults 
       ...
       ||' awrrpt_'||d.name||'_i'||s.instance_number||'_' ||TO_CHAR(s.end_interval_time,'DD_MON_YYYY_HH24MI')||'.html'
...
AND s.end_interval_time>= (trunc(sysdate, 'HH')-(2/24))
AND s.end_interval_time<=(sysdate)
-- AND s.end_interval_time>=to_date('2024/03/07 23:00','YYYY/MM/DD HH24:MI')
-- AND s.end_interval_time<=to_date('2024/03/08 04:00','YYYY/MM/DD HH24:MI')
ORDER BY s.snap_id, s.instance_number; 

spool off 

spool fndawrbatch.sql append
...
!cat fndawrbatch.sql
PRO
pause Review above list and Press any key to continue...
